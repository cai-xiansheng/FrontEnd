			...
			<div class="mui-card-content">
				<div class="mui-card-content-inner">
					...
					<p>
						...
						<!-- 1. 经过分析发现：按钮属于 goodsinfo 页面，数字数与 numberbox 组件 -->
...
// 导入数组选择框组件
import number from '../subcomponents/goodsinfo_numberbox.vue'


export default {
	data() {
		return{
			id: this.$route.params.id, // 将路由参数中的 ID 挂载到 data ，方便后期调用
			lunbotu: [], // 轮播图的数据
			goodsinfo: [], // 商品参数
			ballFlag: false, // 控制小球出现和隐藏的标识符
	...
	methods: {
		getLunbotu() {
			this.$http.get('api/getthumimages/' + this.id).then( result => {
				if(result.body.status === 0) {
					// 先循环伦比图数组的每一项，为 item 添加 img 属性，因为轮播图组件中，只认识 item.img ,不认识 item.src
		...
		addToShopCar() {
			...
			// { id:商品的id,count:要购买的数量,price:商品的价格,select: false}
			// 拼接出一个要保存到 store 中的 car 数组里的商品信息对象
			var goodsinfo = { 
				id: this.id, 
		...
		getSelectedCount(count) {
			// 当子组件把选中的数量传递给都组件的时候，把选中的式保存到 data 上
